...
export async function insertUser(userData: { id: string; email: string; name: string; role: User['role']; password?: string }) {
  // 管理员创建用户：默认密码（可在创建后重置）
  const password = userData.password || 'default123';
  return signUp(userData.email, password, userData.name, userData.role);
}
...
export async function createBorrowRecord(recordData: Omit<BorrowRecord, 'id' | 'created_at' | 'updated_at'>) {
  return request<BorrowRecord>('/api/borrow-records?action=create', {
    ...
    body: JSON.stringify({
      ...
      borrower_name: recordData.borrower_name,
      borrower_contact: recordData.borrower_contact,
